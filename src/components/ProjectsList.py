...
class ProjectsList(tb.Frame):
    def __init__(self, parent, app):
        ...
        self.buttons = [
            ProjectListItem(
                frm_list_selection, self.app, project, self.selected_project_id
            )
        ...
        btn_new_project = tb.Button(
            self,
            text="New",
            bootstyle="success",
            command=self.new_project_frame,
        )
        ...
        self.project_detail_panel.grid(
            row=0, rowspan=3, column=2, sticky="nsew"
        )
    ...
    def __update_screen(self, *args):
        ...
        new_project = next(
            filter(
                lambda el: el.id == self.selected_project_id.get(),
                self.projects,
            ),
            None,
        )
    ...
    def new_project_frame(self):
        ...
        self.new_project_frame.grid(
            row=0, rowspan=3, column=0, columnspan=3, sticky="nsew"
        )
...
class ProjectDetailPanel(tb.Frame):
    def __init__(self, parent, app, project):
        ...
        lbl_heading = tb.Label(
            self, textvariable=self.project_name, font=(None, 36, "bold")
        )
        lbl_heading.grid(
            row=0, column=0, columnspan=2, padx=lbl_padx, pady=25, sticky="ew"
        )

        self.btn_edit_save = tb.Button(
            self, text="Edit", command=self.__select_handler, bootstyle="info"
        )
        self.btn_edit_save.place(relx=0.98, rely=0.02, anchor="ne")

        lbl_category_heading = tb.Label(self, text="Category:")
        lbl_category_heading.grid(
            row=1, column=1, padx=lbl_padx, pady=lbl_pady, sticky="w"
        )
        inp_category_value = tb.Combobox(
            self,
            textvariable=self.project_category,
            font=COMBO_BOX_FONT,
            state="disabled",
        )
        inp_category_value.grid(
            row=2, column=1, padx=lbl_padx, pady=inp_pady, sticky="nsew"
        )

        lbl_description_heading = tb.Label(self, text="Description:")
        lbl_description_heading.grid(
            row=3, column=1, padx=lbl_padx, pady=lbl_pady, sticky="w"
        )
        self.inp_description_value = ScrolledText(
            self, height=4, state="disabled"
        )
        if self.project is not None:
            self.inp_description_value.text.insert(
                1.0, self.project.description
            )
        self.inp_description_value.grid(
            row=4, column=1, padx=lbl_padx, pady=inp_pady, sticky="nsew"
        )

        lbl_activities_heading = tb.Label(self, text="Activities:")
        lbl_activities_heading.grid(
            row=5, column=1, padx=lbl_padx, pady=lbl_pady, sticky="w"
        )
        self.activities_list = ActivitiesList(self, self.project)
        self.activities_list.grid(
            row=6, column=1, padx=lbl_padx, pady=inp_pady, sticky="nsew"
        )
...
class ActivitiesList(tb.Frame):
    ...
    def build_list(self, project):
        self.frm_activities = ScrolledFrame(
            self, height=120, scrollheight=120, borderwidth=1, relief="solid"
        )
        self.frm_activities.grid(row=0, column=0, sticky="nsew")

        for activity in project.activities:
            lbl_activity = tb.Label(self.frm_activities, text=activity.name)
...
class NewProjectForm(tb.Frame):
    def __init__(self, parent):
        ...
        inp_project_category = tb.Combobox(
            self, textvariable=self.category_id_var, font=inp_font
        )
