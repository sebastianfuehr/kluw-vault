...
class TimeEntryForm(tb.Frame):
    ...
    def build_form_components(self):
        ...
        lbl_te_date.grid(column=0, row=0, sticky='w', padx=10, pady=5)
        self.te_date = tb.Entry(self, font=self.entry_font, width=14, justify='center')
        self.te_date.grid(column=1, row=0, sticky='e', padx=10, pady=5)

        lbl_te_weekday = tb.Label(self, text='Day:')
        lbl_te_weekday.grid(column=0, row=1, sticky='w', padx=10, pady=5)
        self.te_weekday = tb.Label(self, width=14, anchor='center')
        self.te_weekday.grid(column=1, row=1, sticky='e', padx=10, pady=5)
        self.te_weekday['text'] = datetime.today().strftime('%a')

        lbl_te_start = tb.Label(self, text='Start:')
        lbl_te_start.grid(column=0, row=2, sticky='w', padx=10, pady=5)
        self.te_start = tb.Entry(self, font=self.entry_font, width=14, justify='center')
        self.te_start.grid(column=1, row=2, sticky='e', padx=10, pady=5)

        lbl_te_end = tb.Label(self, text='End:')
        lbl_te_end.grid(column=0, row=3, sticky='w', padx=10, pady=5)
        self.te_end = tb.Entry(self, font=self.entry_font, width=14, justify='center')
        self.te_end.grid(column=1, row=3, sticky='e', padx=10, pady=5)

        lbl_te_pause = tb.Label(self, text='Pause:')
        lbl_te_pause.grid(column=0, row=4, sticky='w', padx=10, pady=5)
        self.te_pause = tb.Entry(self, font=self.entry_font, width=14, justify='center')
        self.te_pause.grid(column=1, row=4, sticky='e', padx=10, pady=5)

        lbl_te_duration = tb.Label(self, text='Duration:')
        lbl_te_duration.grid(column=0, row=5, sticky='w', padx=10, pady=5)
        self.te_duration = tb.Label(self, width=14, anchor='center')
        self.te_duration.grid(column=1, row=5, sticky='e', padx=10, pady=5)
        ...
        lbl_te_project.grid(column=0, row=6, sticky='w', padx=10, pady=5)
        self.te_project = tb.Combobox(self,
                                      textvariable=self.selected_project,
                                      font=self.entry_font,
                                      width=12,
                                      justify='right')
        self.te_project.grid(column=1, row=6, sticky='e', padx=10, pady=5)
        ...
        lbl_te_activity.grid(column=0, row=7, sticky='w', padx=10, pady=5)
        self.te_activity = tb.Combobox(self,
                                       textvariable=self.selected_activity,
                                       font=self.entry_font,
                                       width=12,
                                       justify='right')
        self.te_activity.grid(column=1, row=7, sticky='e', padx=10, pady=5)
