...
@pytest.fixture
def default_time_entry() -> TimeEntry:
...
def test_get_weekday(default_time_entry: TimeEntry) -> None:
    assert default_time_entry.get_weekday_str() == "Fr"


def test_get_pause_timedelta(default_time_entry: TimeEntry) -> None:
    assert default_time_entry.get_pause_timedelta() == timedelta(seconds=62)


def test_get_duration_timedelta(default_time_entry: TimeEntry) -> None:
    assert default_time_entry.get_duration_timedelta() == timedelta(seconds=3666)


def test_get_duration_minutes(default_time_entry: TimeEntry) -> None:
    assert default_time_entry.get_duration_minutes() == 3666 / 60


def test_eq(default_time_entry: TimeEntry) -> None:
    assert default_time_entry == default_time_entry
    start = datetime(2023, 7, 21, 10, 10, 10)
    stop = datetime(2023, 7, 21, 11, 12, 18)
    project = Project(id=1, name="Test Project")
    activity = Activity(id=0, name="Test Activity")
...
def test_to_list(default_time_entry: TimeEntry) -> None:
    project = Project(id=1, name="Test Project")
    activity = Activity(id=0, name="Test Activity")
...
def test_from_list(default_time_entry: TimeEntry) -> None:
    project = Project(id=1, name="Test Project")
    activity = Activity(id=0, name="Test Activity")
    te_list = [
        260,
    ...
    te_from_list: TimeEntry = TimeEntry.from_list(te_list)
